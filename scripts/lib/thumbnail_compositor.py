...
def clean_text_for_pillow(text):
    """Normalizes input text by converting stylized mathematical bold/italic
    Unicode letter variants back into plain ASCII, and strips any remaining
    non-ASCII characters/emoji so Pillow never renders a 'tofu' box for a
    glyph its bundled font doesn't have."""
    if not text:
        return ""
...
def draw_headline(base: Image.Image, words, zone, font_path: Path):
    ...
    cleaned_words = []
    for w in words:
        text = clean_text_for_pillow(w.get("text", ""))
        if text:
...
def draw_category_banner(base: Image.Image, label: str, font_path: Path, accent=(237, 28, 36)):
    label = clean_text_for_pillow(label)
    if not label:
        return base
    draw = ImageDraw.Draw(base)
