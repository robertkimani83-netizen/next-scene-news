...
// Single reusable image component for every place an article photo is
// shown (homepage hero, story cards, article pages, related stories).
// Handles three things in one place:
//   1. The VOX254 watermark overlay on every real photo.
//   2. The branded category placeholder when no real photo was found
//      (photo.url is empty - see lib/photos.ts fallback hierarchy).
//   3. Broken-image recovery - if the photo URL fails to load at display
//      time, falls back to the branded placeholder instead of showing a
//      broken image icon.
...
interface ArticleImageProps {
  ...
  alt: string;
  className?: string;
  aspectRatio?: string; // e.g. "16/9", defaults to filling the container
